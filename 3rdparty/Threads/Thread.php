...
class Thread
{
   ...
   */
  public function __construct( $runnable = null, $arguments=[] )
  {
  ...
  public function start()
  {
    ...
    if ( $pid ) {
      ...
      while(strlen($results)>0) {
        if (($written = socket_write($ary[1], $results, strlen($results))) === false) {
          die("SOCKET ERR: " . socket_last_error());
        }
        if ($written==strlen($results)){
          break;
        }
        $results = substr($results, $written);
      }
      socket_close($ary[1]);
      exit( 0 );
    }
  ...
  public function getResults(){
    $result='';
    if ($this->ary!==null) {
      while(($data = socket_read($this->ary, 10 * 1024 * 1024, PHP_BINARY_READ))!==false) {
        if ($data != '') {
          $result .= $data;
          if (json_decode($result,1)!==false){
            $this->results = json_decode($result,1);
            socket_close($this->ary);
            $this->ary = null;
            break;
          }
        }
