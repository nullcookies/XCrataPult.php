...
class Generator
{
     ...
     *                           $wordLists array of word lists or word list
  	 * @param  integer           $length    password length in words. Default - 4
  	 * @param  string            $separator word separator. Default - ' '(space)
  	 * @return string                       generated password
  	 */
    public static function generate($wordLists, $length = 4, $separator = ' ')
    {
        ...
        $words = array();
        $randomArray = self::getRandomArray($length);
     ...
     * 
     * @param  integer $length    password length (number of words). Default - 4
     * @param  string  $separator word separator. Default ' ' (space)
     * @return string             generated password
     */
    public static function generateRuTranslit($length = 4, $separator = ' ')
    {
        $adjectives = new WordList\RuTranslit\Adjectives();
        $nouns = new WordList\RuTranslit\Nouns();
        $verbs = new WordList\RuTranslit\Verbs();
        
        return self::generate(array($adjectives, $nouns, $verbs, $nouns), $length, $separator);
    }
     ...
     * @return string             generated password
     */
    public static function generateRu($length = 4, $separator = ' ')
    {
        $adjectives = new WordList\Ru\Adjectives();
        $nouns = new WordList\Ru\Nouns();
        $verbs = new WordList\Ru\Verbs();
        
        return self::generate(array($verbs, $adjectives, $nouns, $verbs, $nouns), $length, $separator);
    }
     ...
     * @return string             generated password
     */
    public static function generateEn($length = 4, $separator = ' ')
    {
